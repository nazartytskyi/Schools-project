...
class SearchPage extends React.Component {
  ...
  filterSchools(filters) {
    ...
    if (filters.znoRange !== undefined) {
      filteredSchools = filteredSchools.filter(school => {
        return (
          school.avgZno >= filters.znoRange[0] &&
          school.avgZno <= filters.znoRange[1]
        );
      });
    }
    ...
    if (filters.feedbackRange) {
      filteredSchools = filteredSchools.filter(school => {
        return (
          this._getAvgFeedbackRate(school) >= filters.feedbackRange[0] &&
          this._getAvgFeedbackRate(school) <= filters.feedbackRange[1]
        );
      });
    }
    ...
    if (filters.language) {
      filteredSchools = filteredSchools.filter(school => {
        return school.language
          .toUpperCase()
          .includes(filters.language.toUpperCase());
      });
    }
